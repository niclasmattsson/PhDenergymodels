...
subject to Cum_invcost {k in TECH, t in TIME}:
  cum_invcost[k,t] >= << {P in 1..npieces[k]-1} bpexper[k,P]; 
    {P in 1..npieces[k]} bpinvcost[k,P] / 1000 >> (exper[k,t],start_exper[k]);
...
#subject to Exper_grows_1 {k in TECH, t in TIME, T in TIME: ord(T)=ord(t)+1}:
#  exper[k,T] >= exper[k,t];
